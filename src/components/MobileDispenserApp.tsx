...
import { LogOut, Fuel, MapPin, User, DollarSign, CheckCircle, RefreshCw, ArrowLeft, AlertTriangle, Receipt, Search, SlidersHorizontal, ArrowUpDown, X, Calendar, ClipboardList } from 'lucide-react';
...
export const MobileDispenserApp: React.FC = () => {
  ...
  // States
  const [mobileTab, setMobileTab] = useState<'dispenser' | 'transactions'>('dispenser');
  ...
  const [viewedTx, setViewedTx] = useState<any | null>(null);
  ...
  return (
    <div className="min-h-screen w-full bg-[#0b0f19] text-slate-100 flex items-center justify-center font-sans">
      <div className="max-w-md w-full min-h-screen bg-[#0d1321] border-x border-slate-800 flex flex-col justify-between shadow-2xl relative">
          ...
          {mobileTab === 'transactions' ? (
            ...
            <div className="space-y-4 text-left animate-fade-in">
              <div className="space-y-1">
                <h3 className="text-sm font-black text-slate-200 uppercase tracking-wider">Transaction Records</h3>
                <p className="text-[10px] text-slate-500 font-semibold leading-relaxed">View and query historical fuel dispensing receipts for this station.</p>
              </div>
              ...
              <div className="space-y-2">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search TX ID, operator, customer..."
                    value={txSearchText}
                    onChange={(e) => setTxSearchText(e.target.value)}
                    className="w-full bg-[#111827] border border-slate-800 rounded-xl py-2.5 pl-10 pr-4 text-xs font-semibold text-white focus:outline-none focus:border-[#8c7dfc] placeholder-slate-600"
                  />
                  {txSearchText && (
                    <button 
                      onClick={() => setTxSearchText('')} 
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 bg-[#111827] border border-slate-800 rounded-xl text-[9px] font-black uppercase text-slate-400 hover:text-white"
                  >
                    <ArrowUpDown size={11} className="text-[#8c7dfc]" />
                    <span>Sort: {txSortOrder === 'newest' ? 'Newest First' : 'Oldest First'}</span>
                  </button>

                  <select
                    value={txGradeFilter}
                    onChange={(e) => setTxGradeFilter(e.target.value)}
                    className="flex-1 bg-[#111827] border border-slate-800 rounded-xl px-3 py-2 text-[9px] font-black uppercase text-slate-400 outline-none focus:border-[#8c7dfc]"
                  >
                    ...
                    {sorted.map(tx => {
                      ...
                      return (
                        <div
                          ...
                          className="bg-[#111827] border border-slate-800 hover:bg-[#161f33] rounded-xl p-3.5 flex justify-between items-center transition-all cursor-pointer shadow-xs"
                        >
                          <div className="space-y-1.5 flex-1 min-w-0 pr-3">
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-mono font-bold text-slate-350 truncate">{tx.id.toUpperCase()}</span>
                            ...
                            <div className="flex items-center gap-1.5 text-[9px] text-slate-500 font-semibold">
                              ...
                              <span className="truncate">{tx.pumpId === 'DELIVERY_BAY' ? 'Delivery' : `Pump ${tx.pumpId.slice(-2)}`}</span>
                              <span>•</span>
        ...
        <nav className="sticky bottom-0 bg-[#111827] border-t border-slate-850 grid grid-cols-2 z-30">
          <button
            onClick={() => {
              setMobileTab('dispenser');
              setCompletedReceipt(null);
              setSelectedPump(null);
            }}
            className={`py-3 flex flex-col items-center justify-center gap-0.5 border-none cursor-pointer transition-all select-none ${
              mobileTab === 'dispenser'
                ? 'text-[#8c7dfc] bg-[#1a2333]/40'
                : 'text-slate-500 hover:text-slate-350'
            }`}
          >
            <Fuel size={16} className={mobileTab === 'dispenser' ? 'text-[#8c7dfc]' : 'text-slate-500'} />
            <span className="text-[9px] font-black uppercase tracking-wider">Dispenser</span>
          </button>
          ...
          >
            <Receipt size={16} className={mobileTab === 'transactions' ? 'text-[#8c7dfc]' : 'text-slate-500'} />
            <span className="text-[9px] font-black uppercase tracking-wider">Transactions</span>
          </button>
        </nav>

        {/* Realtime Live Sync Status */}
        <div className="bg-[#0b0f19] py-1 border-t border-slate-900 text-center select-none z-30">
          <div className="inline-flex items-center justify-center gap-1.5 text-[8px] font-mono text-slate-600 uppercase tracking-widest leading-none">
            <span className="w-1 h-1 bg-[#48bb78] rounded-full animate-pulse"></span>
            <span>Cloud Realtime Live Connection</span>
          </div>
        </div>
        ...
        {viewedTx && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-xs p-4 animate-fade-in">
            <div className="bg-[#111827] border border-slate-800 rounded-xl overflow-hidden shadow-2xl p-5 space-y-4 max-w-sm w-full font-mono text-xs font-semibold relative">
              
              {/* Close Button */}
              <button
                onClick={() => setViewedTx(null)}
                className="absolute top-3.5 right-3.5 p-1 bg-slate-900 hover:bg-slate-850 rounded-full border border-slate-800 text-slate-400 hover:text-white transition-colors cursor-pointer select-none"
              >
                <X size={14} />
              </button>
              ...
              <div className="space-y-2 border-b border-dashed border-slate-800 pb-4">
                <div className="flex justify-between">
                  <span className="text-slate-555">TX ID:</span>
                  <span className="font-bold text-slate-300 text-[10px] truncate max-w-[150px]">{viewedTx.id.toUpperCase()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-555">TIMESTAMP:</span>
                  <span className="text-slate-350">{viewedTx.timestamp}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-555">PUMP ID:</span>
                  <span className="font-extrabold text-slate-300">
                    {viewedTx.pumpId === 'DELIVERY_BAY' ? 'Replenishment Log' : `Pump ${viewedTx.pumpId.slice(-2)}`}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-555">FUEL GRADE:</span>
                  <span className="font-extrabold text-[#8c7dfc]">{viewedTx.fuelType}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-555">PRICE/LITRE:</span>
                  <span className="text-slate-350">SAR {viewedTx.pricePerLitre ? viewedTx.pricePerLitre.toFixed(2) : '2.18'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-555">ATTENDANT:</span>
                  <span className="text-indigo-300 font-bold">{viewedTx.operator || 'Station Operator'}</span>
                </div>
                {viewedTx.customer && (
                  <div className="flex justify-between">
                    <span className="text-slate-555">CUSTOMER:</span>
                    <span className="text-indigo-300 font-bold truncate max-w-[150px]">{viewedTx.customer}</span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-slate-400">
                  ...
                  <span className="text-slate-200 font-black text-sm">{viewedTx.volume.toFixed(2)} L</span>
                </div>
                <div className="flex justify-between text-emerald-400 pt-1.5 border-t border-slate-900">
                  <span className="font-bold">TOTAL AMOUNT:</span>
                  <span className="font-black text-base">SAR {viewedTx.amount.toFixed(2)}</span>
                </div>
              </div>

              <button
                onClick={() => setViewedTx(null)}
                className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-[10px] font-black uppercase tracking-wider transition-all shadow-md select-none cursor-pointer border-none text-center font-sans mt-2"
              >
                Close Receipt
              </button>
            </div>
